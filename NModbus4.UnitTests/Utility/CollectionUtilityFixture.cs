...
namespace Modbus.UnitTests.Utility
{
    public class CollectionUtilityFixture
    {
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => MessageUtility.CreateDefaultCollection<RegisterCollection, short>(0, -1));
        }

        [Fact]
        public void CreateDefaultCollection()
        {
            RegisterCollection col = MessageUtility.CreateDefaultCollection<RegisterCollection, short>(3, 5);
            Assert.Equal(5, col.Count);
            Assert.Equal(new short[] {3, 3, 3, 3, 3}, col.ToArray());
        }
    }
}
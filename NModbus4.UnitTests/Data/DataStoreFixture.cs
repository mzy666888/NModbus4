...
namespace Modbus.UnitTests.Data
{
    public class DataStoreFixture
    {
        ...
        {
            ModbusDataCollection<short> slaveCol = new ModbusDataCollection<short>(0, 1, 2, 3, 4, 5, 6);
            RegisterCollection result = DataStore.ReadData<RegisterCollection, short>(new DataStore(), slaveCol, 1, 3,
                new object());
            Assert.Equal(new short[] {1, 2, 3}, result.ToArray());
        }
        ...
        public void TestReadMapping()
        {
            ...
            Assert.Equal(45,
                DataStore.ReadData<RegisterCollection, short>(dataStore, dataStore.HoldingRegisters, 0, 1, new object())
                    [0]);
            Assert.Equal(42,
                DataStore.ReadData<RegisterCollection, short>(dataStore, dataStore.HoldingRegisters, 1, 1, new object())
                    [0]);
        }
        ...
        public void DataStoreReadFromEvent_ReadHoldingRegisters()
        {
            ...
            dataStore.DataStoreReadFrom += (obj, e) =>
            {
                ...
                Assert.Equal(new short[] {4, 5, 6}, e.Data.B.ToArray());
                Assert.Equal(ModbusDataType.HoldingRegister, e.ModbusDataType);
            };

            dataStore.DataStoreWrittenTo += (obj, e) => writtenToEventFired = true;

            DataStore.ReadData<RegisterCollection, short>(dataStore, dataStore.HoldingRegisters, 3, 3, new object());
        ...
        public void DataStoreReadFromEvent_ReadInputRegisters()
        {
            ...
            dataStore.DataStoreReadFrom += (obj, e) =>
            {
                ...
                Assert.Equal(new short[] {}, e.Data.B.ToArray());
                Assert.Equal(ModbusDataType.InputRegister, e.ModbusDataType);
            };

            dataStore.DataStoreWrittenTo += (obj, e) => writtenToEventFired = true;

            DataStore.ReadData<RegisterCollection, short>(dataStore, dataStore.InputRegisters, 4, 0, new object());
            Assert.True(readFromEventFired);
            Assert.False(writtenToEventFired);
        }
        ...
        public void DataStoreWrittenToEvent_WriteHoldingRegisters()
        {
            ...
            dataStore.DataStoreWrittenTo += (obj, e) =>
            {
                ...
                Assert.Equal(new short[] {5, 6, 7}, e.Data.B.ToArray());
                Assert.Equal(ModbusDataType.HoldingRegister, e.ModbusDataType);
            };

...
namespace Modbus.UnitTests.Device
{
    public class ModbusMasterFixture
    {
        ...
        public void WriteMultipleRegisters()
        {
            ...
            Assert.Throws<ArgumentException>(() => master.WriteMultipleRegisters(1, 1, new short[] {}));
            Assert.Throws<ArgumentException>(
                () => master.WriteMultipleRegisters(1, 1, Enumerable.Repeat<short>(1, 124).ToArray()));
        }
        ...
        public void ReadWriteMultipleRegisters()
        {
            ...
            Assert.Throws<ArgumentException>(() => master.ReadWriteMultipleRegisters(1, 1, 0, 1, new short[] {1}));
            Assert.Throws<ArgumentException>(() => master.ReadWriteMultipleRegisters(1, 1, 126, 1, new short[] {1}));

            // validate writeData
            Assert.Throws<ArgumentNullException>(() => master.ReadWriteMultipleRegisters(1, 1, 1, 1, null));
            Assert.Throws<ArgumentException>(() => master.ReadWriteMultipleRegisters(1, 1, 1, 1, new short[] {}));
            Assert.Throws<ArgumentException>(
                () => master.ReadWriteMultipleRegisters(1, 1, 1, 1, Enumerable.Repeat<short>(1, 122).ToArray()));
        }
    }
}